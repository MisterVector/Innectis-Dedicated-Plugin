...
import net.innectis.innplugin.location.skyandspopulators.SkylandsPlusCactusPopulator;
import net.innectis.innplugin.location.skyandspopulators.SkylandsPlusFlowerPopulator;
import net.innectis.innplugin.location.skyandspopulators.SkylandsPlusGrassPopulator;
import net.innectis.innplugin.location.skyandspopulators.SkylandsPlusLakePopulator;
import net.innectis.innplugin.location.skyandspopulators.SkylandsPlusMushroomPopulator;
...
/**
 * This was taken from Jacek's SkyLandsPlus plugin
 *
 * @author Jacek
 */
public class SkylandsPlusChunkGenerator extends ChunkGenerator {
	...
	byte liquid_id;
        ...
        public SkylandsPlusChunkGenerator() {
            // IDP start
            no_desert = true;
            desert = Biome.PLAINS;
            // IDP end
	...
	public SkylandsPlusChunkGenerator(String id){
		...
		for (int i = 0; i < tokens.length; i++){
			if (tokens[i].matches("offset={1}-?\\d{1,3}")){
				...
				liquid_id = (byte) MagicValueUtil.getIdFromBukkitMaterial(Material.WATER);
			}else if (tokens[i].matches("lava={1}\\d{1,3}")){
				liquid = Integer.parseInt(tokens[i].substring(5)) + 1;
				liquid_id = (byte) MagicValueUtil.getIdFromBukkitMaterial(Material.LAVA);
			}
		}
	}
	...
	public byte[][] generateBlockSections(World world, Random random, int chunkX, int chunkZ, BiomeGrid biomes){
		...
		for (int x = 0; x < 16; ++x){
			for (int y = 0 + cut_bottom; y < 128 - cut_top; ++y){
				for (int z = 0; z < 16; ++z){
					chunk[y + offset >> 4][((y + offset & 0xF) << 8) | (z << 4) | x] = ((Integer) Block.getId(blocks[(x * 16 + z) * 128 + y])).byteValue();
					if (bedrock == true && y == 0){
						chunk[y - cut_bottom >> 4][((y - cut_bottom & 0xF) << 8) | (z << 4) | x] = (byte) MagicValueUtil.getIdFromBukkitMaterial(Material.BEDROCK);
					}
